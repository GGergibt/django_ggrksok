...
from .models import insta_profiles, user
...
def insta_download(account_name: str, folder: str, current_user=None):
    try:
        ...
        insta_account = insta_profiles.objects.get(name=account_name)
        if current_user != None:
            insert_info_about_stored_profiles = user.objects.get(
                username=current_user
            ).stored_profiles.add(insta_account)
        return "OK"
